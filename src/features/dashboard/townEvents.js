...
//
// The day's character follows the child's ACTIVE super skill (plus Daniel
// for variety) — see townEventData.js, which the daily-reminders edge
// function mirrors so the morning push names the same character.
// ================================================
...
import { CHARS, dateKey, pickEvent as pickPoolEvent } from './townEventData.js'

const FEELINGS = ['😄 Happy', '😌 Calm', '😐 Okay', '😟 Worried', '😢 Sad', '😠 Cross']

function todayKey() { return dateKey(new Date()) }

// The child's active super skill slug, or null when the gate can't be
// computed yet (data still loading) — the pool then falls back to the
// full character rotation.
function activeSkillSlug() {
  try { return getSkillGate()?.activeSlug || null } catch { return null }
}
...
export function tomorrowEventChar(childId) {
  ...
  const ev = pickPoolEvent(childId, dateKey(t), activeSkillSlug())
  const ch = CHARS[ev.c]
...
function pickEvent(childId) {
  return pickPoolEvent(childId, todayKey(), activeSkillSlug())
}
